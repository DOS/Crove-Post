      ...
      {!!mapData?.length && !selected && (
        <>
          <div
            ...
            onClick={() => setName('')}
          />
          <div className="absolute top-[100%] start-0 w-max min-w-full max-w-[90vw] bg-sixth border border-newBgLineColor text-textColor z-[999]">
            {mapData.map((item: any) => (
              <div
                onClick={pick(item)}
                key={item?.id}
                className="p-[10px] border-b border-newBgLineColor hover:bg-tableBorder cursor-pointer whitespace-nowrap truncate"
              >
                {t('user_1', 'user:')}
                {item?.id?.split('-')?.at(-1)} -{' '}
                {item?.name ? `${item?.name} - ` : ''}
          ...
          {!!data?.length && (
            <>
              <div
                ...
                onClick={() => setName('')}
              />
              <div className="absolute top-[100%] w-max min-w-full max-w-[90vw] start-0 bg-sixth border border-newBgLineColor text-textColor z-[999]">
